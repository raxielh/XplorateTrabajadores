...
FROM 
  ...
  order by rh1.fechaencuesta,rh1.respuestas_historic_1, pe.cedula,ca.orden, pr.orden,pre.orden;";
    ...
    {
        $sql="
SELECT pc.campus,pc.`acad_prog`,pc.`tipoempleado`,te.`desctipoempleado`,ac.`desccargo`,count(*) as numero
FROM    b_rowdata_1 r1 ,
        b_persona_categoria pc  ,
              b_tipoempleado te,
        b_cargo ac 
where r1.fechaencuesta between '$request->fi 00:00:01' and '$request->ff 23:59:59'
AND R1.`persona`=PC.`cedula`
and pc.`cargo`=ac.`cargo`
and pc.`tipoempleado`=te.`tipoempleado`
GROUP BY pc.campus,pc.`acad_prog`,pc.`tipoempleado`,te.`desctipoempleado`,ac.`desccargo` 
ORDER BY pc.campus,pc.`acad_prog`,pc.`tipoempleado`,te.`desctipoempleado`,ac.`desccargo` ";
